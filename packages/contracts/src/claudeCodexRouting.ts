...
/**
 * One step of a bridge sign-in. Exactly one of `browserHandoff` / `deviceCode`
 * follows `started`, and the stream always ends with `completed` or `failed`
 * unless the client unsubscribes first (unsubscribing IS the cancel).
 */
export const ClaudeCodexBridgeSignInEvent = Schema.Union([
