...
import { CodexAccountService } from "./CodexAccountService.ts";
...
function makeInstance(auth: ProviderAuthOps | undefined): ProviderInstance {
  return {
    ...
    ...(auth === undefined ? {} : { accountAuth: auth }),
...
function makeLayer(input: {
  ...
  return CodexAccountService.layer.pipe(Layer.provide(Layer.mergeAll(instanceRegistry, registry)));
}
...
describe("CodexAccountService.startSignIn", () => {
  it.effect("refreshes the instance exactly once on a completed login", () =>
    Effect.gen(function* () {
      ...
      const events = yield* Effect.gen(function* () {
        const service = yield* CodexAccountService;
...
describe("CodexAccountService.signOut", () => {
