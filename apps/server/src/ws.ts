...
import { CodexAccountService } from "./provider/Services/CodexAccountService.ts"; // fork: f1 provider account sign-in
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const codexAccount = yield* CodexAccountService; // fork: f1 provider account sign-in
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.providerStartSignIn]: (input) =>
          observeRpcStream(WS_METHODS.providerStartSignIn, codexAccount.startSignIn(input), {
            "rpc.aggregate": "provider",
          }),
        // fork: f1 provider account sign-out
        [WS_METHODS.providerSignOut]: (input) =>
          observeRpcEffect(WS_METHODS.providerSignOut, codexAccount.signOut(input), {
            "rpc.aggregate": "provider",
          }),
